...
const RestaurantsPage = () => {
    const {data: restaurants} = useQuery({queryKey: ["restaurants"], queryFn: () => RestaurantsApi.getRestaurants(new FormData())})
    ...
    return <div className="py-20 lg:px-20 px-10" style={{
        ...
        {(restaurants ||[]).map((restaurant) => <CardRestaurant key={restaurant.id} restaurant={restaurant} />)}
      </div>
    </div>
}
