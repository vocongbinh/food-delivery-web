...
const BlogPage = ({ params }: { params: { id: number } }) => {
    const { data: BlogData, isLoading } = useCustomQuery<Blog>({
        key: "blogs", id: params.id
    });
    const { data: relatedBlogs } = useCustomQuery<Blog>({
        key: "blogs",
        opts: {
            enabled: BlogData !== undefined
        }
    })
    if (isLoading) {
        return <></>
        ...
        return <div>
            ...
            <div className="container py-5 bg-\[\#f8f8f8\]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                    {(relatedBlogs as Blog[] || [])
                        .map((item, index) => (
                            <CardNews key={index} blog={item} />
                        ))}
                </div>
            </div>
        </div>;
