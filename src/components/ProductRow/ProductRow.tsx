...
export function ProductRow({ number, name, graphSrc, percentage, color }: ProductRowProps) {
  const colorStyles = {
  ...
  return (
    <div className="flex gap-5 justify-between items-center self-center mt-2 w-full text-xs font-medium dark:text-white text-black max-md:max-w-full">
      ...
      /> */}
      <Progress className="w-[50%]" percent={percentage} strokeColor={color} trailColor='#2B2B36' showInfo={false} />
      <div className={`px-2.5 py-1.5 whitespace-nowrap rounded border border-solid w-[10%] ${colorStyles[color]} bg-opacity-10`}>
        {percentage}%
      </div>
    </div>
  );
