...
export interface CardApplication2Props {
  className?: string;
  app: Application;
  index?: number;
}

const CardApplication2: FC<CardApplication2Props> = ({
  className = "h-full",
  app,
  index
}) => {
  ...
  return (
    <div
      className={`nc-CardApplication2 relative flex flex-col  hover:bg-opacity-50 group p-3 items-center rounded-3xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 ${className}`}
    >
      <div className="w-1/3 flex-shrink-0 relative">
        <Link
          href={`${href}?id=${id}`}
          className="block h-0 aspect-w-1 aspect-h-1 relative rounded-2xl overflow-hidden shadow-lg"
        >
          <Image
      ...
      <div className="flex flex-col flex-grow mt-4 items-center">
        ...
        <span className="text-base text-neutral-500 dark:text-neutral-400 my-1 text-center">
          {subTitle}
        </span>
